...
class TileDownloader():
    ...
    def get_composite(self, sc_stack, is_landsat8):
        ...
        rgb_tuple = (composite_rgb_img, "rgb/composite.png")
        ...
        if self._download_ir:
            ...
            ir_tuple = (composite_ir_img, "ir/composite.npy")
        else:
            ir_tuple = None
    ...
    def write_images(self, single_rgb_imgs, single_ir_imgs,
        ...
        for rgb_img, rgb_path in single_rgb_imgs + composite_rgb_img:
            rgb_img.save(os.path.join(images_path, rgb_path))
            if 'composite' not in rgb_path: years_lst.append(int(rgb_path.split('/')[-1].split('_')[0]))

        # Write all IR images
        for ir_img, ir_path in single_ir_imgs + composite_ir_img:
    ...
    def download_ls8_images(self, lat, lon, year, images_path):
        ...
        print(unique, counts, year, year_cts)
    ...
    def download_ls7_images(self, lat, lon, year, images_path):
...
def download_images(meta_load_path,
    ...
    for index, row in tqdm(metadata.iterrows(), total=metadata.shape[0]):
        ...
        # if year + 1 < 2012: continue #only comparing post-2012
        if len(total_cts) % 20 == 0:     
            # mean_cts, std_cts = np.mean(total_cts, 0), np.std(total_cts, 0)
            np.save('./total_cts.npy', np.array(total_cts))
...
def main():
    ...
    total_cts, num_not_found = download_images(DATA_BASE_DIR / f'{split}_curtis.csv', 
        DATA_BASE_DIR / f'images_curtis/{split}/', 
        DATA_BASE_DIR / f'postdownload_meta_{split}_v4.csv',
        download_ls7=True)
    with open(DATA_BASE_DIR / f'log_v4_{split}.txt', 'w') as f:
        f.write(f'Did not find {num_not_found}/{len(total_cts)} images')
    np.save(DATA_BASE_DIR / f'scene_cts_v4_{split}.npy', total_cts)
