...
from util.util import get_num_channels, copy_weights
...
class TorchVisionModel(PretrainedModel):
    ...
    def __init__(self, model_fn, model_args):
        ...
        self.pool = nn.AdaptiveAvgPool2d(1)
        self.model_name = model_args['model'].lower()

        num_outputs = model_args.num_classes

        if 'fc' in self.model.__dict__['_modules']:
            num_ftrs = self.model.fc.in_features
            self.model.fc = nn.Linear(num_ftrs, num_outputs)
        elif 'classifier' in self.model.__dict__['_modules']:
            ...
            raise RuntimeError(
                'Last layer of ' +
                model_args['model'] +
                ' was not resized to num_outputs: ' +
                str(num_outputs))

        num_channels = get_num_channels(model_args)
        if not model_args["composite"]:
            if "densenet" in self.model_name:
                conv_weight = self.model.state_dict()["features.conv0.weight"]
                new_conv_layer = nn.Conv2d(num_channels, C.NUM_INPLANES,
                                           kernel_size=7, stride=2, padding=3,
                                           bias=False)
                self.model.features.conv0 = new_conv_layer
            elif "resnet" in self.model_name:
                conv_weight = self.model.state_dict()["conv1.weight"]
                new_conv_layer = nn.Conv2d(num_channels, C.NUM_INPLANES,
                                           kernel_size=7, stride=2, padding=3,
                                           bias=False)
                self.model.conv1 = new_conv_layer
            else:
                raise RuntimeError('Only DenseNet and ResNet implemented')

        if model_args["pretrained"] and not model_args["composite"]:
            copy_weights(self.model, conv_weight, num_channels, self.model_name)
