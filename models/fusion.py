...
class FusionNet(nn.Module):
    def __init__(self, model, model_args):
        ...
        if self.late_fusion_regions == 'none':
            ...
            num_loc_features = self.N * 4 #20
        elif self.late_fusion_regions == 'onehot':
            num_loc_features = len(C.REGIONS) #7
        else: 
            raise Exception(f'Invalid late_fusion_regions type entered! : {self.late_fusion_regions}')
        ...
        num_aux_features = self.use_aux * len(self.aux_features) # 0 if self.use_aux False
         
        ## I/O dims
        self.input_dim = num_logit_features + num_loc_features \
        + num_polygon_loss_features + num_aux_features
        
        self.output_dim = self.hparams["num_classes"]  
        
        self.pool = nn.AdaptiveAvgPool1d(self.cnn_logits_dim)

        self.cnn2embs = nn.Linear(DN_NFEATS, num_logit_features)
    ...
    def get_logits(self, batch):
        x = self.model.features(batch['image'])
